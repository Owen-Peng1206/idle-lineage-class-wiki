...
1. **確認目前 wiki 追蹤的遊戲主程式版本 (Old Commit)**:
   在主專案 (wiki) 目錄下執行以下指令，找出目前紀錄的 submodule commit hash：
   ```bash
   git ls-tree HEAD idle-lineage-class
   ```
   (記下輸出的 commit hash，這是更新前的版本)
...
6. **紀錄更新日誌 (Changelog)**:
   - 在專案根目錄下找到 (或建立) `CHANGELOG.md` 檔案。
   - 將本次更新的遊戲版本號（可從 `idle-lineage-class/js/00-data.js` 的 `GAME_VERSION` 擷取）、日期，以及這次 Wiki 主要跟進了哪些修改內容條列出來，寫入 `CHANGELOG.md` 的最上方。
